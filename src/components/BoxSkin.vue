<template>
	<div>
		<div  style="display: none;" class="bslist">
			<div class="left">
				<div class="boxbox">
					<div class="imgbox">
					...
					<div class="px-4 d-flex flex-column j-center h112">
						<div class="fs22 line1">{{box?.box_name || '月光宝盒'}}</div>
						<div class="d-flex align-center" v-if="type !== 'welfare'">
							<img class="mr-1 douzi" src="../assets/img/bos/douzi.svg" alt="">
		...
		<div class="slist">
			<div v-for="(item,index) in skinList" :key="index" class="sitem">
...
<script setup lang="ts">
	...
	const getSkinList = (id,roomId) => {
	  boxId = id;
	  state.loading = true;
	 return BoxService.boxSkin({
	   box_id: boxId,
	   page_size: 999,
	 })
	    .then((response) => {
	      state.skinList = response.data.data.list.sort((a, b) => b.price - a.price);
