...
export function useCode() {
  ...
  let timer = null
  const { buttonText } = toRefs(state);
  function sendPhoneCode(phone, ticket, randStr) {
    if (state.loading) return
    if (state.buttonText != '获取验证码') {
      return;
    }
    state.loading = true;
    UserService.sendPhoneCode({
      mobile: phone,
	  ticket: ticket,
	  rand_str: randStr
    })
  ...
  function startCountdown() {
    state.buttonText = state.n-- + "秒后重新获取";
    if (state.n > 0) {
