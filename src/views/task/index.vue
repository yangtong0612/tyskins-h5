<template>
    <div>
        <v-container>
            <div class="task-tabs">
                <div class="tab-item" @click="onChangeTask(index)" v-for="(title, index) in titles" :key="index"
                    :class="{ active: index == titleIndex }">
                    {{ title.label }}
                </div>
                <div v-if="!isMobile" class="right">
                    <span> 我的积分： <span v-if="store.getters.userInfo" class="text-yellow fs14 fw600">{{
                        store.state.user?.integral }}</span></span>
                    <div @click="goLottery" class="btn">
                        去抽奖
                    </div>
                </div>
            </div>

            <div v-loading="!loaded" class="task-list-content" style="min-height: 400px;">
                <div v-if="isMobile" class="text-white d-flex mb-4">
                    <span> 我的积分： <span v-if="store.getters.userInfo" class="text-yellow fs14 fw600">{{
                        store.state.user?.integral }}</span></span>
                    <div @click="goLottery" class="qbtn">
                        去抽奖
                    </div>
                </div>
                    ...
                    <div class="task-item" v-for="item in taskList" :key="item.id">
                        <div class="d-flex a-center">
                            <div class="flex-1">
                                ...
                                <div class="desc">
                                    <span v-if="item.cycle == 'month'" class="mr-8">时间： {{ dateTimes['month'].start_time }}
                                        - {{ dateTimes['month'].end_time }}</span>
                                    <span v-if="item.cycle == 'week'" class="mr-8">时间： {{ dateTimes['week'].start_time }} -
                                        {{ dateTimes['week'].end_time }}</span>
                                    <span v-if="item.reward.type == 'diamond'">赠送：<img
                                            style="width: 12px; margin-right: 5px;" src="@/assets/img/bos/suipian.svg"
                                            alt=""> {{ item.reward.value }} </span>

                                    <span v-else-if="item.reward.type == 'coin'">赠送： <img
                                            style="width: 12px; margin-right: 5px;" src="@/assets/img/bos/douzi.svg" alt="">
                                        {{ item.reward.value }}</span>

                                    <span v-else-if="item.reward.type == 'integral'">赠送： <img
                                            style="width: 14px; margin-right: 5px;" src="@/assets/img/bos/icon_integral.png"
                                            alt=""> {{ item.reward.value }}</span>
                ...
                <div v-if="loaded" class="roll-loading d-flex justify-center align-center">
                    <el-result icon="info">
                        <template #title>
...
import { defineComponent, toRefs, reactive, onMounted, watch, inject, computed } from "vue";
