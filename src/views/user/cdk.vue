<template>
	<div>
		<div v-show="!isMobile" class="d-flex a-center space-between mb-6">
			<div class="flex-1 text-white" style="font-size: 22px">我的CDK</div>
			<v-btn
		...
		<!-- CDK表格 -->
		<div class="cdk-table-wrapper mt-6">
			...
			<div v-if="cdkList.length > 0">
				<div v-for="(item, index) in cdkList" :key="index" class="table-row py-3">
					<v-row class="fs14">
						...
						<v-col cols="2" class="tc">
							<v-btn
								...
								size="small"
							>
								<span class="fs12 text-white">删除</span>
							</v-btn>
...
const { cdkList, page, page_size, total, showGenerateDialog, generating, generateForm } = toRefs(state);
...
const handleGenerate = async () => {
	...
	if (!state.generateForm.value || state.generateForm.value <= 0) {
		ElMessage.warning('请输入有效的价值');
		return;
	}
	if (!state.generateForm.quantity || state.generateForm.quantity <= 0) {
	...
	try {
		const res = await CdkService.generate(state.generateForm);
		if (res.data.code === 0) {
