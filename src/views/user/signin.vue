  ...
  <div class="record">
    ...
    <div class="mb-4">
      ...
      <div v-else class="wd-mtitle">
        <span style="width: 112px">累计签到可获得</span>
      </div>
    </div>
    <el-table
      :data="signinConfigList"
      ...
      <el-table-column label="条件（等级/累充）" prop="point">
        <template v-slot="scope">
          <template v-for="(item, index) in scope.row.condition">
            ...
            <span
              class="ml-1 mr-1"
              v-if="index != scope.row.condition.length - 1"
              >/</span
            >
          </template>
        </template>
      ...
      <el-table-column label="奖励" prop="point">
        <template v-slot="scope">
          <div v-for="(item, index) in scope.row.reward" :key="index">
            <span v-if="item.type == 'key'" class="td-text"
              >{{ item.name }}X1</span
            >
    ...
    <el-table
      ...
      header-cell-class-name="custom-header"
    >
      <el-table-column label="签到次数" prop="days"> </el-table-column>
    ...
    >
      <div class="fs14 py-6 px-4">
        ...
        <p class="mb-2" style="color: #333333; font-size: 13px">
          2、签到奖励发放以签到当时是否满足所有条件为准，后补足条件不予补发。
        </p>
        <div class="mb-2" style="color: #333333; font-size: 13px">
          ...
          <div style="padding-left: 20px">
            ...
            <p>
              如果你等级已经达到lv.50那么你每月无需充值，签到28天即可领取4把钥匙
            </p>
          </div>
        </div>
        <p class="mb-2" style="color: #333333; font-size: 13px">
