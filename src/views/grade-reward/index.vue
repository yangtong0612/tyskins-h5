  ...
  <div class="grade-wrap">
    <v-container class="container-box">
        ...
        <div class="flex-1">
          ...
          <div>
            ...
            ><span class="userLevel">
              ...
              <i>LV.</i>{{ userInfo?.grade }}</span
            >
          </div>
          <div class="mt-3" v-if="levelRewardList.length">
            <el-progress
              :percentage="`${
                (userInfo?.recharge_total /
                  calcRequiredMoney(levelRewardList)) *
                100
              }`"
            >
              {{ userInfo?.recharge_total }} /
      ...
      <div
        ...
        class="level-openbox-container a-center mt-10"
      >
        <div class="fs14 tc mb-4 text-white">
          充值金额升级获取奖励，赠送宝箱在我的盲盒里。
        </div>
        <div :class="isMobile ? 'm-level-openbox' : 'level-openbox'">
          <div
            class="box-item"
            :class="{ active: userInfo.grade >= level.end_level }"
            v-for="(level, idx) in levelRewardList"
            :key="idx"
          >
            <div class="boximg">
              <img
                style="display: block; width: 100%; height: 100%"
                src="@/assets/img/bos/boxbg.png"
                alt=""
              />
              <img
                v-if="level?.extend?.boxInfo"
                class="coverImg"
                :src="processImageUrl(level?.extend?.boxInfo.cover)"
                alt=""
              />
              <img
                v-else
                class="coverImg"
                :src="processImageUrl(level?.extend?.image_url)"
                alt=""
              />
              <img
                v-if="level?.extend?.boxInfo"
                class="boxCoverImg"
                :src="processImageUrl(level?.extend?.boxInfo.img)"
                alt=""
              />
              ...
              <p v-if="level?.extend?.boxInfo" class="line1 fs12">
                {{ level?.extend?.type_name }}
              </p>
              <p v-else class="line1 fs12 subtitle">
                {{ level?.extend?.item_name }}
              </p>
              <div class="tips" v-if="level.reward_type === 'skin'">
                <v-btn height="24" color="#f3a45d" class="btn" elevation="0">{{
                  verifyIsLock(userInfo.grade, level.end_level)
                    ? "已领取到背包"
                    : "未解锁"
                }}</v-btn>
              </div>
              <div class="tips" v-else>
                <v-btn
                  ...
                  @click="
                    openBox(
                      verifyIsLock(userInfo.grade, level.end_level),
                      level?.extend?.boxInfo?.id,
                      level?.extend?.image_url
                    )
                  "
                  >{{
                    verifyIsLock(userInfo.grade, level.end_level)
...
const { inviteCode, levelSetting, record, userInfo, levelRewardList } =
  toRefs(state);
