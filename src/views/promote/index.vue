		...
		<v-container class="container-box">
			<div class="promote-topbg">
				...
				<div class="level-line">
					<div :class="index ==4? 'active':''" v-for="(item,index) in 9" :key="index"><span>LV.{{ userInfo?.level || 0 }}</span></div>
				</div>
				<div style="margin: 0 auto;" class="stats d-flex space-between tc text-white w70">
					<div class="stat-item">
					...
					<div class="stat-item">
						<div>
							...
							<v-btn @click="receiveRebate" class="ml-1" elevation="0" color="primary" height="24" type="primary" v-if="stat.rebate_current">领取</v-btn>
						</div>
						<div style="color:#99A5B7;" class="fs14">待领返利</div>
					</div>
				</div>
				<div style="margin-left: auto; margin-right: auto;" class="d-flex space-between px-4 py-4 mt-12 w80 mhide">
					<div class="d-flex" style="align-items: center;">
						<span class="fs13 mr-2 text-white tglable">你的推广链接</span>
						<section class="d-flex">
							<div class="fs13 pl-2 inpbg line1 tglzinp">
								{{ `${BASE_URL}/?code=${inviteCode}` }}</div>
							<v-btn style="border-radius: 0 10px 10px 0px" @click="copyText(`${BASE_URL}/?code=${inviteCode}`)" class="ml-1" elevation="0" color="#f3a55e"
								height="34">复制</v-btn>
						</section>
					</div>
					<div class="d-flex" style="align-items: center;">
						...
						<section class="d-flex">
							<div  class="inpbg fs13 pl-2 tglzinp">
								{{ inviteCode }}</div>
							<v-btn style="border-radius: 0 10px 10px 0px" @click="copyText(inviteCode)" class="ml-1" elevation="0" color="#f3a55e" height="34">复制</v-btn>
						</section>
					</div>
				</div>
			</div>
		...
		<v-container class="container-box2">
			<div class="mt-5">
				<div v-if="!isMobile"  class="d-flex">
					...
					<span class="text-yellow fs12">(下级累计充值金额:{{stat.recharge}})</span>
				</div>

				<div class="wd-mtitle">
					<span>充值金额</span>
					<div class="desc" style="color: #FFB300;">（下级累计充值金额：{{stat.recharge}}）</div>
				</div>

				<div class="mt-6 mb-2 fs14" style="color: #ffffff; background-color: rgba(255,255,255,0.1);">
					<v-row class="fs12">
						<v-col cols="1"></v-col>
				...
				<div v-for="(item, index) in levelSetting.list" :key="index" class="py-3 bor_b">
					<v-row class="fs14">
						<v-col cols="1"></v-col>
						<v-col cols="3" class="tc"><span class="text-white">Lv.{{ item?.level }}</span></v-col>
						<v-col cols="3" v-if="index + 1 < levelSetting.list.length">
							<span class="text-white">{{ `${item.recharge_amount}-${levelSetting.list[index + 1].recharge_amount - 1}` }}</span></v-col>
						<v-col cols="3" v-else><span class="text-white">{{ `${item.recharge_amount}-` }}</span></v-col>
						<v-col cols="3"><span class="text-white">{{ item.rebate_rate }}%</span></v-col>
					</v-row>
				</div>
			</div>
			<div class="mt-6">
				...
				<div v-for="(item, index) in record" :key="index" class="py-3">
					<v-row class="fs14 text-gray">
						<v-col cols="3" class="tc"><span class="text-white">{{ item.nickname }}</span></v-col>
						<v-col cols="3"><span class="text-white">{{ item.recharge_total }}</span></v-col>
						<v-col cols="3"><span class="text-white">{{ $dayjs.unix(item.created_at).format("YYYY-MM-DD") }}</span></v-col>
						<v-col cols="3"><span class="text-white">{{ item.rebate_total }} 金币</span></v-col>
					</v-row>
				</div>
			</div>
			<div class="d-flex py-15 justify-center">
				<el-pagination :page-size="state.page_size" :total="state.total" background layout="prev, pager, next"
...
.tglzinp{
	height: 34px; width:294px;line-height: 34px;
	border-radius: 10px;
}
